    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-900 font-display">IRSP Works</h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2 p-1.5 rounded-xl bg-slate-100/50 border border-slate-200/40">
            <Filter size={14} className="text-slate-400 ml-3" />
            <Select value={selectedYear} onValueChange={setSelectedYear}>
              <SelectTrigger className="w-[160px] h-10 bg-transparent border-0 focus:ring-0 text-xs font-bold text-slate-600 uppercase tracking-widest font-display">
              ...
              {filteredWorks.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-8 py-32 text-center">
                    ...
                    <h3 className="text-2xl font-bold text-slate-800 font-display tracking-tight">System holds no IRSP records</h3>
                ...
                filteredWorks.map((work) => (
                  <tr key={work.id} className="premium-table-row transition-all duration-500 group border-l-4 border-l-transparent hover:border-l-sky-500">
                    <TableCell className="px-8 py-6 font-bold text-slate-800 text-sm leading-snug group-hover:text-sky-600 transition-colors font-display">{work.description || '-'}</TableCell>
                    <TableCell className="px-6 py-6 text-center">
                      <span className="px-3 py-1 bg-slate-50 text-[11px] font-bold font-display text-slate-500 rounded-lg border border-slate-100">{work.yearOfSanction || '-'}</span>
                    </TableCell>
                    <TableCell className="px-6 py-6 text-center">
                    ...
                    <TableCell className="px-6 py-6">
                      <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-[10px] font-bold border uppercase tracking-wider transition-all font-display ${getStatusStyles(work.status)}`}>
                    ...
                    <TableCell className="px-6 py-6">
                      <div className="flex items-baseline gap-1">
                        ...
                        <span className="text-base font-bold text-slate-900 font-display tracking-tight">{work.totalValue || '-'}</span>
                      </div>
                    </TableCell>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent className="rounded-[2.5rem] border-none shadow-[25px_25px_60px_rgba(0,0,0,0.15)] bg-white p-10 max-w-lg">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogTitle className="text-3xl font-display font-bold text-slate-900 spacing-tight">Remove record?</AlertDialogTitle>
